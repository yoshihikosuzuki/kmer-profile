...
class PreadVisualizer:
    ...
    def add_counts(
        ...
        show_init_markers: bool = True,
    ) -> PreadVisualizer:
        pread = self.pread
        t = (
            None
            if pread._seq is None or pread.K is None
            else [
        ...
        return self.add_traces(
            [
                ...
                for mode, col, show_init in [
                    ("lines", col_lines, show_init_lines),
    ...
    def add_states(
        ...
        if self.pread.states is None:
            return self
        state_pos = [
            list(filter(lambda i: self.pread.states[i] == s, self.pos_list))
            for s in STATES
        ]
        return self.add_traces(
            [
        ...
        if show_asgn:
            ...
            for i, (b, e, cb, ce, _, s) in enumerate(data):
                for j, (p, c) in enumerate([(b, cb), (e, ce)]):
                    ...
                    t[idx] = S_TO_COL[s]
            traces.append(pl.scatter(x=x, y=y, col=t))
        return self.add_traces(traces)

    def add_simple_intvls(
        self,
                ...
                filter(
                    ...
                    [
                        trace_minus(
                            data,
                            col,
                            f"{ctype} length (>={min_val}) [{wtype}]",
                            min_val,
                            use_webgl,
                            show_legend,
                            show_init,
                        )
                        for ctx, ctype, ctx_cols, min_val in zip(
                            ctx_by_type, ctx_names, cols, min_vals
                        )
    ...
    def add_walls(
        ...
        return self.add_traces(
            pl.lines(
                [(i, 0, i, max_count) for i in pread.walls],
                text=[
                    (
                        f"{pread.counts[i - 1]}@{i - 1}"
                        if i > 0
                        else "" + f"->{pread.counts[i]}@{i}" if i < pread.length else ""
                    )
    ...
    def show(
        ...
        fixed_layout = pl.layout(title=self.title, width=self.width, height=self.height)
        fig = pl.figure(
            self.traces, pl.merge_layout(default_layout, layout, fixed_layout)
        )
        if max_count_zoom is not None and (
            self.max_count is None or max_count_zoom < self.max_count
        ):
            fig.update_layout(
                updatemenus=[
                    dict(
