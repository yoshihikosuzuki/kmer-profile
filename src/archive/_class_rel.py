...
def classify_rel(pread, cp, verbose):
    ...
    # multi-pass version (currently inefficient)
    # cr_f, df, hf, hdrrf = classify_rel_fw_it(pread, cp, verbose)
    # cr_b, db, hb, hdrrb = classify_rel_bw_it(pread, cp, verbose)

    # By the difference between first count and last count
    ...
    # By R-mer rate (using only this is inaccurate)
    # ri_f = sum([I.e - I.b for s, I in zip(cr_f.asgn, pread.rel_intvls) if s == 'R'])
    # ri_b = sum([I.e - I.b for s, I in zip(cr_b.asgn, pread.rel_intvls) if s == 'R'])
    # cr = cr_f if ri_f <= ri_b else cr_b
...
def classify_rel_fw_it(pread, cp, verbose):
    if verbose:
        print(f"first F {cp.depths}")
    cr_f, df, hf, hdrrf = classify_rel_fw(pread, cp, verbose)
    asgn = cr_f.asgn
    for I, s in zip(pread.rel_intvls, asgn):
        I.asgn = s
    d_intvls = list(filter(lambda I: I.asgn == 'D', pread.rel_intvls))
    h_intvls = list(filter(lambda I: I.asgn == 'H', pread.rel_intvls))
    if len(d_intvls) == 0 and len(h_intvls) == 0:
        return (cr_f, df, hf, hdrrf)
    _cp = deepcopy(cp)
    if len(d_intvls) > 0 and len(h_intvls) > 0:
        _cp.depths['D'] = d_intvls[-1].cce
        _cp.depths['H'] = h_intvls[-1].cce
    elif len(d_intvls) > 0:
        _cp.depths['D'] = d_intvls[-1].cce
        _cp.depths['H'] = max(_cp.depths['D'] - cp.depths['H'], int(_cp.depths['D'] / 2))
    else:
        _cp.depths['H'] = h_intvls[-1].cce
        _cp.depths['D'] = _cp.depths['H'] + cp.depths['H']

    if verbose:
        print(f"first B {_cp.depths}")
    cr_b, db, hb, hdrrb = classify_rel_bw(pread, _cp, verbose)
    asgn = cr_b.asgn
    for I, s in zip(pread.rel_intvls, asgn):
        I.asgn = s
    d_intvls = list(filter(lambda I: I.asgn == 'D', pread.rel_intvls))
    ...
    if len(d_intvls) == 0 and len(h_intvls) == 0:
        return (cr_f, df, hf, hdrrf)
    if len(d_intvls) > 0 and len(h_intvls) > 0:
        _cp.depths['D'] = d_intvls[0].ccb
        _cp.depths['H'] = h_intvls[0].ccb
    elif len(d_intvls) > 0:
        _cp.depths['D'] = d_intvls[0].ccb
        _cp.depths['H'] = max(_cp.depths['D'] - cp.depths['H'], int(_cp.depths['D'] / 2))
    else:
    ...
    if verbose:
        print(f"second F {_cp.depths}")
    return classify_rel_fw(pread, _cp, verbose)
...
def classify_rel_bw_it(pread, cp, verbose):
    if verbose:
        print(f"first B {cp.depths}")
    cr_f, df, hf, hdrrf = classify_rel_bw(pread, cp, verbose)
    asgn = cr_f.asgn
    for I, s in zip(pread.rel_intvls, asgn):
        I.asgn = s
    d_intvls = list(filter(lambda I: I.asgn == 'D', pread.rel_intvls))
    ...
    if verbose:
        print(f"first F {_cp.depths}")
    cr_b, db, hb, hdrrb = classify_rel_fw(pread, _cp, verbose)
    asgn = cr_b.asgn
    for I, s in zip(pread.rel_intvls, asgn):
        I.asgn = s
    d_intvls = list(filter(lambda I: I.asgn == 'D', pread.rel_intvls))
    ...
    if verbose:
        print(f"second B {_cp.depths}")
    return classify_rel_bw(pread, _cp, verbose)
